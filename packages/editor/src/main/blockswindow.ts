...
import { exceptionToString } from "../error";
...
export class BlocksWindow {
    ...
    private async generate() {
        const [output, error]: string[] = await this.doMenuAction(MenuAction.GENERATE);
        if (error === null) {
            if (this.currentFile.filename != null) {
                const filename = this.currentFile.filename + ".js";
    ...
    private async compile() {
        ...
        if (result != undefined && this.currentFile.filename != null) {
            const [generated, filename] = result;
            const [output, error] = await this.doMenuAction(MenuAction.COMPILE, filename, generated);
            if (error === null) {
                try {
                    await writeFile(`${filename}.zr16`, output.mne);
                    await writeFile(`${filename}.zr16.bin`, output.binary);
                    await writeFile(`${filename}.zr16.mne`, output.mne);
                } catch (e: unknown) {
